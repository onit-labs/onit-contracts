...
abstract contract ForumGroupGovernance is NftReceiver {
        ...
        notPaused
    {
        ...
        require(
            to.code.length == 0
                ? to != address(0)
                : NftReceiver(to).onERC1155Received(msg.sender, from, id, amount, data) == NftReceiver.onERC1155Received.selector,
            "UNSAFE_RECIPIENT"
        );
    }
    ...
    function _mint(address to, uint256 id, uint256 amount, bytes memory data) internal {
        ...
        require(
            to.code.length == 0
                ? to != address(0)
                : NftReceiver(to).onERC1155Received(msg.sender, address(0), id, amount, data) == NftReceiver.onERC1155Received.selector,
            "UNSAFE_RECIPIENT"
        );
    }
    ...
    function _batchMint(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal {
        ...
        require(
            to.code.length == 0
                ? to != address(0)
                : NftReceiver(to).onERC1155BatchReceived(msg.sender, address(0), ids, amounts, data)
                    == NftReceiver.onERC1155BatchReceived.selector,
            "UNSAFE_RECIPIENT"
        );
    }
