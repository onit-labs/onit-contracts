...
contract PfpStaker is IPfpStaker, ReentrancyGuard, Owned, NFTreceiver {
     ...
     */
    function stakeNFT(address staker, address NftContract, uint256 tokenId)
        external
    {
    ...
    function getURI(address staker, string calldata groupName)
        external
        view
        returns (string memory)
    {
        ...
        if (stake.Nftcontract == address(0)) {
            ...
            if (IERC721(stake.Nftcontract).supportsInterface(ERC721_METADATA)) {
                image =
                    IERC721Metadata(stake.Nftcontract).tokenURI(stake.tokenId);
            } else {
                // Check if NftContract is ERC1155
                if (
                    IERC1155(stake.Nftcontract).supportsInterface(ERC1155_METADATA)
                ) {
                    image =
                        IERC1155MetadataURI(stake.Nftcontract).uri(stake.tokenId);
                }
            }
        }
    ...
    {
        (NftContract, tokenId) =
            (stakes[staker].Nftcontract, stakes[staker].tokenId);
    }
        ...
        return JSON._formattedMetadata(
            ...
            string.concat(
                ...
                SVG._image(
                    image,
                    string.concat(
                        SVG._prop("x", "215"), SVG._prop("y", "220"), SVG._prop("width", "80")
                    )
                ),
                "</svg>"
            )
    ...
    function unstakeNFT() internal {
        if (
            ...
            IERC721(stakes[msg.sender].Nftcontract).safeTransferFrom(
                THIS_ADDRESS, msg.sender, stakes[msg.sender].tokenId
            );
        } else {
            if (
                ...
                IERC1155(stakes[msg.sender].Nftcontract).safeTransferFrom(
                    THIS_ADDRESS, msg.sender, stakes[msg.sender].tokenId, 1, ""
                );
            }
        }
    }
