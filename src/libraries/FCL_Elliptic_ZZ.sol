...
/// @author Forum - Modified from FCL
/// Changes:
/// - Removed currently unsupported verification functions (ecdsa_precomputed_hackmem & ecdsa_precomputed_verify)
/// - Removed currently unused functions & inlined others to reduce code size
/// - Converted ecdsa_verify to external to use an external library
library FCL_Elliptic_ZZ {
     ...
     */
    function ecdsa_verify(bytes32 message, uint256[2] calldata rs, uint256[2] calldata Q) external returns (bool) {
        ...
        bool ecAff_isOnCurve;
        if (0 == Q[0] || Q[0] == p || 0 == Q[1] || Q[1] == p) {
            return false;
        }
        unchecked {
            uint256 LHS = mulmod(Q[1], Q[1], p); // y^2
            uint256 RHS = addmod(mulmod(mulmod(Q[0], Q[0], p), Q[0], p), mulmod(Q[0], a, p), p); // x^3+ax
            RHS = addmod(RHS, b, p); // x^3 + a*x + b

            ecAff_isOnCurve = LHS == RHS;
        }
        if (!ecAff_isOnCurve) {
            return false;
        }
