...
import {IPfpSetter} from '@interfaces/IPfpSetter.sol';
...
contract PfpSetter is IPfpSetter, NFTreceiver {
	...
	function stakeNft(address staker, uint256 tokenId) external {
		...
		if (!(msg.sender == staker || msg.sender == pfpStore)) revert Unauthorized();

		if (ERC1155(pfpStore).balanceOf(staker, tokenId) == 0) revert NotTokenHolder();
		...
		if (tokenId == 0) {
			...
			ERC1155(pfpStore).safeTransferFrom(staker, THIS_ADDRESS, tokenId, 1, '');
	...
	function getUri(
		...
		if (stake == 0) {
			...
			image = ERC1155(pfpStore).uri(stake);
		}
	...
	function _buildURI(
		...
		return
			JSON._formattedMetadata(
				...
				string.concat('Forum Group', tokenId == 0 ? ' GovernanceToken' : ''),
				string.concat(
					...
					SVG._text(
						...
						SVG._cdata(tokenId == 0 ? 'GovernanceToken' : '')
					),
					'</svg>'
				)
			);
	...
	function unstakeNFT(address staker) private {
		ERC1155(pfpStore).safeTransferFrom(THIS_ADDRESS, staker, stakedNft[staker], 1, '');
	}
}
