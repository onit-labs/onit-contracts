    ...
    IERC1271
{
     ...
     */
    mapping(address => mapping(bytes32 => uint256)) private
        contractSignatureAllowance;
        ...
        nonReentrant
    {
        ...
        if (govSettings_[3] < 1 || govSettings_[3] > 100) revert
            VoteThresholdBounds();
        ...
        )
    {
        ...
        (accounts, amounts, payloads) =
            (prop.accounts, prop.amounts, prop.payloads);
    }
    ...
    {
        if (
            accounts.length != amounts.length || amounts.length != payloads.length
        ) revert NoArrayParity();

        if (proposalType == ProposalType.VPERIOD) if (
            amounts[0] == 0 || amounts[0] > 365 days
        ) revert PeriodBounds();

        if (proposalType == ProposalType.MEMBER_LIMIT) if (
            amounts[0] > 100 || amounts[0] < memberCount
        ...
        if (proposalType == ProposalType.TYPE) if (
            amounts[0] > 13 || amounts[1] > 2 || amounts.length != 2
        ) revert TypeBounds();

        if (proposalType == ProposalType.MINT) if (
            (memberCount + accounts.length) > memberLimit
        ) revert MemberLimitExceeded();
        ...
        emit NewProposal(
            msg.sender, proposal, proposalType, accounts, amounts, payloads
            );
    }
        ...
        returns (bool didProposalPass, bytes[] memory results)
    {
        ...
        for (uint256 i; i < signatures.length;) {
            ...
            address recoveredSigner =
                ecrecover(digest, signatures[i].v, signatures[i].r, signatures[i].s);
            ...
            if (voteType == VoteType.MEMBER) votes +=
                1 + EnumerableSet.length(memberDelegators[recoveredSigner]); // Else we calculate the number of votes based on share of the treasury
            else {
                uint256 len =
                ...
                if (len != 0) for (uint256 j; j < len;) {
                    votes += balanceOf[EnumerableSet.at(
                        memberDelegators[recoveredSigner], j
                    )][TOKEN];
                    ++j;
                }
            }
        ...
        if (didProposalPass) {
            ...
            unchecked {
                ...
                if (prop.proposalType == ProposalType.BURN) for (
                    uint256 i; i < prop.accounts.length;
                ) {
                ...
                // Governance settings
                if (prop.proposalType == ProposalType.VPERIOD) votingPeriod =
                    uint32(prop.amounts[0]);

                if (prop.proposalType == ProposalType.MEMBER_LIMIT) memberLimit
                = uint32(prop.amounts[0]);
                ...
                if (prop.proposalType == ProposalType.TYPE) proposalVoteTypes[ProposalType(
                    prop.amounts[0]
                )] = VoteType(prop.amounts[1]);

                if (prop.proposalType == ProposalType.PAUSE) _flipPause();

                if (prop.proposalType == ProposalType.EXTENSION) for (
                    uint256 i; i < prop.accounts.length; i++
                ) {
                    if (prop.amounts[i] != 0) extensions[prop.accounts[i]] =
                        !extensions[prop.accounts[i]];
                ...
                if (prop.proposalType == ProposalType.DOCS) docs =
                    string(prop.payloads[0]);
                ...
                if (prop.proposalType == ProposalType.PFP) {
                    ...
                    IPfpStaker(pfpExtension).stakeNFT(
                        address(this), prop.accounts[0], prop.amounts[0]
                    );
                }
                ...
                if (prop.proposalType == ProposalType.ALLOW_CONTRACT_SIG) {
                    ...
                    for (uint256 i; i < prop.accounts.length; i++) {
                        contractSignatureAllowance[prop.accounts[i]][bytes32(
                            prop.payloads[i]
                        )] = 1;
                    }
                }
                ...
                emit ProposalProcessed(
                    prop.proposalType, proposal, didProposalPass
                    );
    ...
    function _countVotes(VoteType voteType, uint256 yesVotes)
        ...
        returns (bool)
    {
        if (voteType == VoteType.MEMBER) if (
            (yesVotes * 100) / memberCount >= memberVoteThreshold
        ) return true;

        if (voteType == VoteType.SIMPLE_MAJORITY) if (
            yesVotes > ((totalSupply * 50) / 100)
        ) return true;

        if (voteType == VoteType.TOKEN_MAJORITY) if (
            yesVotes >= (totalSupply * tokenVoteThreshold) / 100
        ) return true;
