...
import {ForumGovernance} from './ForumSafe4337Governance.sol';
	...
	Initializable
{
	...
	) internal virtual nonReentrant onlyInitializing {
		(
			string memory _name,
			string memory _symbol,
			address[] memory _extensions,
			uint32[2] memory _govSettings
		) = abi.decode(_initializationParams, (string, string, address[], uint32[2]));
