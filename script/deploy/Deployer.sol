...
import {Script2} from '@lib/Script2.sol';
...
abstract contract Deployer is Script2 {
	...
	GenericFork internal fork = GenericFork(singleton('Deployer:Fork'));
	...
	ToyENS internal remoteEns = ToyENS(address(bytes20(hex'decaf0')));

	bool internal writeDeploy; // whether to write a .json file with updated addresses
	address internal _broadcaster; // who will broadcast
	bool internal forMultisig; // whether the deployment is intended to be broadcast or given to a multisig.
	bytes32 internal salt; // salt used for create2 deployments
	...
	constructor() {
		...
		if (singleton('Deployer:Fork') == address(0)) {
			...
			if (
				address(remoteEns).code.length == 0 &&
				ANVIL_DEFAULT_FIRST_ACCOUNT.balance >= 1000 ether
			) {
				deployRemoteToyENS();
			}
		...
		// ! consider if the below is needed
	...
	function broadcaster() public returns (address) {
		...
		if (_broadcaster == address(0)) {
			...
			if (
				...
				_broadcaster == 0x1804c8AB1F12E6bbf3894d4083f33e07309d1f38 // ! CHECK OTHER BROADCASTER
			) {
				...
				if (envHas('BROADCASTER')) {
					...
					try vm.envUint(pkEnvVar) returns (uint key) {
						...
						console.log(
							'%s not found or not parseable as uint, using default broadcast sender',
							pkEnvVar
						);
					}
				}
	...
	function outputDeployment() internal {
		...
		string memory timestampedBackupFile = fork.addressesFile(
			'deployed.backup',
