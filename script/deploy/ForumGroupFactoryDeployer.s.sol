...
contract ForumGroupFactoryDeployer is DeploymentSelector {
    ...
    function innerRun() public {
        ...
        bytes memory initData = abi.encode(forumGroupSingleton, entryPoint, gnosisSingleton, gnosisFallbackHandler);

        (address contractAddress, bytes memory deploymentBytecode) = SelectDeployment("ForumGroupFactory", initData);
