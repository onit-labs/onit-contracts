...
contract GenericFork is Script {
	...
	ToyENS internal context;
	// addresses we deploy
	ToyENS internal deployed;
	...
	function addressesFile(
		string memory category,
		string memory suffix
	) public view returns (string memory) {
		return
			string.concat(vm.projectRoot(), '/addresses/', category, '/', NETWORK, suffix, '.json');
	}
		...
		try vm.readFile(fileName) returns (string memory addressesRaw) {
			...
			try vm.parseJson(addressesRaw) returns (bytes memory jsonBytes) {
				try (new Parser()).parseJsonBytes(jsonBytes) returns (Record[] memory records) {
					...
					revert(
						string.concat('Fork: JSON to Record[] parsing error on file ', fileName)
					);
				}
			} catch {
