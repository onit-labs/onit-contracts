...
describe('Fundraise', function () {
	...
	it('Should process native `value` fundraise round', async function () {
		// Check initial state
		expect(
			(await fundraise.getFund(forum.address)).individualContribution
		...
		expect(
			(await fundraise.getFund(forum.address)).contributors.length
		).to.equal(2);
		expect(
			(await fundraise.getFund(forum.address)).individualContribution
		).to.equal(getBigNumber(50));
		...
		expect(await hardhatEthers.provider.getBalance(fundraise.address)).to.equal(
			getBigNumber(0)
		);
	...
	it('Should revert if not all members have contributed', async function () {
		await expect(fundraise.processFundRound(forum.address)).revertedWith(
			'MembersMissing()'
		);
	});
	...
	it('Should cancel round only if cancelled by proposer or dao, and return funds', async function () {
		await expect(
			fundraise.connect(alice).cancelFundRound(forum.address)
