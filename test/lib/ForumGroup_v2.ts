...
describe.only('Forum Multisig V2 Setup and Functions', function () {
	...
	describe('Proposals', function () {
		...
		it('Should revert if member proposal is for longer than year', async function () {
			await forum.init(
				'FORUM',
				'FORUM',
				[bob.address],
		...
		it('Should forbid a member from voting again on proposal', async function () {
			...
			await expect(
				forum.processProposal(1, [proposerSig, proposerSig])
		...
		it.skip('Should forbid voting after period ends - skipped, no hard deadline, instead encourage deletion of old proposals', async function () {
			...
			await expect(forum.processProposal(1, [proposerSig])).revertedWith(
				'NotVoteable()'
			);
		});
		...
		it('Should process escape proposal', async function () {
			...
			await forum
				.connect(proposer)
				.propose(10, [proposer.address], [2], [0x00]);
		...
		it('Should process contract call proposal - Single', async function () {
			...
			const ExecutionManager = await hardhatEthers.getContractFactory(
				'ExecutionManager'
			);
			...
			await executionManager
				.connect(proposer)
				.toggleNonCommissionContract(shieldManager.address);

			let payload = shieldManager.interface.encodeFunctionData(
				'mintShieldPass',
				[alice.address]
			);
			await forum.init(
		...
		it('Should process contract call proposal - Multiple', async function () {
			...
			let payload1 = shieldManager.interface.encodeFunctionData(
				'mintShieldPass',
				[alice.address]
			);
			...
			await executionManager
				.connect(proposer)
				.toggleNonCommissionContract(test1155.address);
	...
	describe('Token Management', function () {
		...
		beforeEach(async function () {
			[sender, receiver, extension, nonextension] =
			...
			await forum
				.connect(extension)
				.mintShares(sender.address, TOKEN, getBigNumber(10));
			await forum
				.connect(extension)
				.mintShares(receiver.address, TOKEN, getBigNumber(10));
		...
		it('Should allow a member to transfer shares', async function () {
			...
			expect(await forum.balanceOf(sender.address, TOKEN)).equal(
				getBigNumber(0)
			);
			expect(await forum.balanceOf(receiver.address, TOKEN)).equal(
				getBigNumber(20)
			);
		...
		it('Should allow a member to approve transfers', async function () {
			...
			expect(
				await forum.isApprovedForAll(sender.address, receiver.address)
			).equal(true);
		});
	...
	describe('Multisig Management', function () {
		...
		beforeEach(async function () {
			...
			await forum
				.connect(extension)
				.mintShares(sender.address, TOKEN, getBigNumber(11));
			await forum
				.connect(extension)
				.mintShares(receiver.address, TOKEN, getBigNumber(10));
		});
		...
		it('should check delegates after a delegation', async function () {
			...
			expect(await forum.memberDelegatee(sender.address)).equal(
				receiver.address
			);
		...
		it('should revert if member tries to delegate, while they are delegated to', async function () {
			...
			await expect(
				forum.connect(receiver).delegate(sender.address)
		...
		it('should revert if attempt to remove invald delegator', async function () {
			await expect(
				forum.connect(receiver).removeDelegator(sender.address)
			).revertedWith('InvalidDelegate()');
		});
		...
		it('create a contract (EIP-1271) signature for use by the group', async () => {
			...
			expect(
				await forum.connect(sender).isValidSignature(testData, fullSig)
			).equal('0x1626ba7e');
		});
		...
		it.skip('should revert reentrant calls', async () => {
			const [sender, receiver, extension, nonextension] =
				await hardhatEthers.getSigners();

			const ReentrantMock = await hardhatEthers.getContractFactory(
				'ReentrantMock'
			);
			...
			await forum
				.connect(sender)
				.propose(9, [reentrantMock.address], [1], [0x00]);
			...
			await expect(
				forum.callExtension(reentrantMock.address, 0, '0x00')
			).revertedWith('Reentrancy()');
		});
	});
});
