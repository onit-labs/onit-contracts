...
describe('PFP_Staking Module', () => {
	...
	beforeEach(async () => {
		...
		hardwareGenerator =
			(await deployTestHardwareGenerator()) as TestHardwareGenerator;
	...
	describe('PFP Unit Tests', () => {
		...
		it('allow owner to toggle restricted mode and revert for non owner', async () => {
			...
			await expect(
				pfpStaker.connect(alice).setRestrictedContracts()
		...
		it('allow owner to set allowed nft contracts for pfps and revert for non owner', async () => {
			...
			await expect(
				pfpStaker.connect(alice).setEnabledContract(bob.address)
			).revertedWith('UNAUTHORIZED');
		});
		...
		it('revert if sender is not from address', async () => {
			...
			await expect(
				pfpStaker
					.connect(founder)
					.stakeNFT(alice.address, testShield721.address, 1)
		...
		it('ERC721 stake pfp', async () => {
			...
			await pfpStaker
				.connect(founder)
				.stakeNFT(founder.address, testShield721.address, 1);

			expect((await pfpStaker.stakes(founder.address)).NFTcontract).equal(
		...
		it('ERC1155 stake pfp', async () => {
			...
			await test1155
				.connect(founder)
				.setApprovalForAll(pfpStaker.address, true);

			await pfpStaker
				.connect(founder)
				.stakeNFT(founder.address, test1155.address, 1);
		...
		it.only('ERC721 get tokenURI', async () => {
			...
			expect(await pfpStaker.getURI(founder.address)).equal(
		...
		it('ERC721 unstake a pfp by replacement', async () => {
			...
			await pfpStaker
				.connect(founder)
				.stakeNFT(founder.address, testShield721.address, 2);
		...
		it('ERC1155 unstake a pfp by replacemet', async () => {
			...
			await pfpStaker
				.connect(founder)
				.stakeNFT(founder.address, test1155.address, 2);
	...
	describe('PFP Integration Tests (linked to ForumGroup)', () => {
		...
		beforeEach(async () => {
			...
			await testShield721
				.connect(owner)
				.setRoundtableFactory(forumFactory.address);
		...
		it('Factory creates group, mints shield, and stakes in pfpStaker', async () => {
			...
			expect((await pfpStaker.stakes(daoAddress)).NFTcontract).equal(
				testShield721.address
			);
