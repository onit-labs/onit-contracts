...
describe('Forum Multisig V2 Setup and Functions', function () {
	...
	describe('Proposals', function () {
		...
		it('Should forbid changing member limit beyond bounds, or to below member count', async function () {
			...
			await forum.connect(proposer).propose(MINT, [alice.address], [getBigNumber(1000)], [0x00])
			const proposerSig = await createSignature(0, forum, proposer, {
				proposal: 1
			})
			...
			await expect(
				forum.connect(proposer).propose(MEMBER_LIMIT, [ZERO_ADDRESS], [getBigNumber(1)], [0x00])
			).revertedWith('MemberLimitExceeded()')

			// Fail to change member limit to 101
			await expect(
				forum.connect(proposer).propose(MEMBER_LIMIT, [ZERO_ADDRESS], [getBigNumber(101)], [0x00])
			).revertedWith('MemberLimitExceeded()')
		})
