...
contract WithdrawalTest is ForumModuleTestConfig, SafeTestConfig {
	...
	function setUp() public {
		...
		withdrawalExtension = new ForumWithdrawalExtension(address(withdrawalTransferManager));
		groupShareManager = new TestShareManager();

		setUpModuleWithExtensions();

		// Set mock ERC20 as a withdrawal token, mint shares for alice
		tokens.push(address(mockErc20));
		vm.startPrank(moduleAddress);
		withdrawalExtension.setExtension(abi.encode(tokens, uint256(WITHDRAWAL_START)));
		groupShareManager.mintShares(moduleAddress, alice, TOKEN, 1 ether);
		vm.stopPrank();

		//Set allowances for both tokens
		vm.startPrank(safeAddress);
		mockErc20.approve(address(withdrawalExtension), 1 ether);
		mockErc20_2.approve(address(withdrawalExtension), 1 ether);
		vm.stopPrank();
	}

	function setUpModuleWithExtensions() private {
		//  Set the extensions
		initialExtensions.push(address(withdrawalExtension));
		initialExtensions.push(address(groupShareManager));
	...
	function testCannotSetEmptyWithdrawalsArray() public {
		...
		withdrawalExtension.setExtension(abi.encode(new address[](0), WITHDRAWAL_START));
	}
	...
	function testSetExtension() public {
		...
		assertEq(
			withdrawalExtension.withdrawables(moduleAddress, 0),
			address(mockErc20),
	...
	function testCannotWithdrawBeforeStartTime() public {
		...
		vm.prank(moduleAddress, moduleAddress);
		withdrawalExtension.setExtension(abi.encode(tokens, uint256(WITHDRAWAL_START + 1000)));

		// Check that the withdrawal fails before the deadline, but works after
		vm.startPrank(alice, alice);
		vm.expectRevert(bytes4(keccak256('NotStarted()')));
		forumModule.callExtension(address(withdrawalExtension), 100, '0x00');
		skip(1001);
		forumModule.callExtension(address(withdrawalExtension), 1, '0x00');
	}
	...
	function testCannotWithdrawMoreThanGroupTokenBalance() public {
		...
		forumModule.callExtension(address(withdrawalExtension), 10001, '0x00');
	...
	function testAddTokenToWithdrawals() public {
		...
		vm.prank(moduleAddress, moduleAddress);
		withdrawalExtension.addTokens(newTokens);

		// Check that the withdrawal token is set
		assertEq(
			withdrawalExtension.withdrawables(moduleAddress, 1),
			address(mockErc20_2),
			'Withdrawal token not set'
		);
	}
	...
	function testRemoveTokenFromWithdrawals() public {
		...
		vm.prank(moduleAddress, moduleAddress);
		withdrawalExtension.removeTokens(removalTokens);

		// Check that the withdrawal token is set
		assert(withdrawalExtension.getWithdrawables(moduleAddress).length == 0);
	}
	...
	function testFuzzCallExtensionBasicWithdrawal(uint256 amount) public {
		...
		vm.prank(alice, alice);
		forumModule.callExtension(address(withdrawalExtension), amount, '0x00');

		// Check balances after erc20 withdrawal
		assertEq(mockErc20.balanceOf(safeAddress), (1 ether - redeemedAmount));
	...
	function testCallExtensionBasicMultipleWithdrawalTokens(uint256 amount) public {
		...
		// Check init balances
		assertEq(mockErc20.balanceOf(safeAddress), 1 ether);
		assertEq(mockErc20_2.balanceOf(safeAddress), 1 ether);
		assertEq(mockErc20.balanceOf(alice), 0);
		assertEq(forumModule.balanceOf(alice, TOKEN), 1 ether);
		...
		// Check balances after erc20 withdrawal
		assertEq(mockErc20.balanceOf(safeAddress), (1 ether - redeemedAmount));
		assertEq(mockErc20_2.balanceOf(safeAddress), (1 ether - redeemedAmount));
		assertEq(mockErc20.balanceOf(alice), redeemedAmount);
		assertEq(mockErc20_2.balanceOf(alice), redeemedAmount);
		assertEq(forumModule.balanceOf(alice, TOKEN), (1 ether - redeemedAmount));
	}
	...
	function testSubmitCustomWithdrawal() public {
		...
		vm.prank(alice, alice);
		withdrawalExtension.submitWithdrawalProposal(
			IForumSafeModule(address(forumModule)),
			accounts,
			amounts,
			100
		);
		...
		) = withdrawalExtension.getCustomWithdrawals(moduleAddress, alice);

		assertEq(withdrawalAssets, accounts);
		assertEq(withdrawalAmounts, amounts);
		assertEq(basisPointsToBurn, 100);
	}
	...
	// 	assertEq(mockErc721.balanceOf(safeAddress), 1);
	// 	assertEq(mockErc721.balanceOf(alice), 0);
	// 	assertEq(forumModule.balanceOf(alice, TOKEN), 1 ether);
	// }

	// // A withdrawal of a non approved token via a custom proposal
	// function testProcessCustomWithdrawal(uint256 erc20Amount, uint256 basisPointsToBurn) public {
	// 	vm.assume(erc20Amount <= 1 ether && basisPointsToBurn <= 10000);
	...
	// 	uint256 redeemedAmount = redeemedAmountFromInputAmount(basisPointsToBurn, alice);

	// 	processProposal(1, forumModule, true);

	// 	// Check balances after withdrawals
	// 	assertEq(mockErc20.balanceOf(safeAddress), 1 ether - erc20Amount);
	...
	function redeemedAmountFromInputAmount(
		...
		uint256 tokenAmount = (amount * forumModule.balanceOf(member, TOKEN)) / BASIS_POINTS;

		redeemedAmount =
			(tokenAmount * mockErc20.balanceOf(safeAddress)) /
