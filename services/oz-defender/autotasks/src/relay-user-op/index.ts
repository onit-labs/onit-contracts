// import { AccessManager } from '../../../abis'
...
async function relay(seaport, body, whitelist) {
	const { contractAddress, parameters, signature, gas } = body

	// Decide if we want to relay this request based on a whitelist
	const accepts = !whitelist || whitelist.includes(contractAddress)
	if (!accepts) throw new Error(`Rejected request to ${contractAddress}`)

	// Send meta-tx through relayer to the seaport contract
	const gasLimit = (parseInt(gas) + 50000).toString()

	return await seaport.validate([{ parameters, signature }], { gasLimit })
}

async function handler(event) {
	// Generate relay input params
	const { targetContract, body, whitelist } = await relayInputGenerator(event, abi)
	// Relay transaction
